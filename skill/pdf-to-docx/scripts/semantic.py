...
NUMBERED = re.compile(r'^(\(?\d{1,2}[.)]|[a-z][.)]|[ivxl]+[.)])\s+', re.IGNORECASE)
...
def build(pdf_path, docx_path):
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        first = pdf.pages[0]
        _apply_page_setup(document, pdf.pages, Pt)
        section = document.sections[0]
        frame = {
            'left': section.left_margin.pt,
            'right': section.page_width.pt - section.right_margin.pt,
        }
        running = _running_bands(pdf.pages)
        _apply_running_bands(document, running, pdf.pages, rasters, frame, summary)
...
PAGINATION = re.compile(r'page|পৃষ্ঠা', re.IGNORECASE)
JOINED = '-/.:_#'
...
def _running_bands(pages):
    ...
    for page in pages:
        for side, lines in _band_lines(page).items():
            ...
            if not key.strip():
                continue
            seen = counts[side].setdefault(key, {'pages': 0, 'lines': lines})
            seen['pages'] += 1
    running = {}
    ...
    for side, candidates in counts.items():
        for key, seen in candidates.items():
            if seen['pages'] >= needed:
                running[side] = {'key': key, 'lines': seen['lines']}
                break
    return running
...
def _apply_running_bands(document, running, pages, rasters, frame, summary):
    ...
    for side, part, flag in (
        ...
        _build_page(part, page.crop(band), summary, frame, rasters.get(1, ()))
        _fieldify(part, 1, len(pages))
        summary[flag] = True
...
def _band_cut(page, side):
    ...
    for obj in list(page.chars) + list(page.images or []):
        ...
        if side == 'top' and top < depth and bottom <= limit:
            edges.append(bottom)
        elif side == 'bottom' and bottom > height - depth and top >= height - limit:
            edges.append(top)
    if not edges:
        return None
...
def _fieldify(part, page_number, total_pages):
    ...
    for paragraph in part.paragraphs:
        for run in list(paragraph.runs):
            if not PAGINATION.search(run.text or ''):
                continue
...
def _build_page(document, page, summary, frame, embedded=(), running=None):
    ...
    ordered = (
        ...
        + [{'kind': 'image', 'top': row[0]['top'], 'region': row} for row in _picture_rows(images)]
    )
    ...
    for entry, space in zip(ordered, spacing):
        if entry['kind'] == 'text':
            _add_text_block(document, entry['block'], body_size, frame, summary, space)
...
def _group_paragraphs(lines, body_size):
    ...
    for line in lines:
        ...
        if previous is not None:
            ...
            starts_item = bool(BULLET.match(line['text']) or NUMBERED.match(line['text']))
...
def _segments(block):
    ...
    for word in block['lines'][0]['words']:
        ...
        if parts and x0 - parts[-1]['x1'] <= TAB_GAP:
            parts[-1]['x1'] = x1
            parts[-1]['words'].append(word['text'])
            continue
        parts.append({'x0': x0, 'x1': x1, 'words': [word['text']]})
    return parts if len(parts) > 1 else None


def _add_tabbed_line(paragraph, block, segments, frame, Pt):
    for index, segment in enumerate(segments):
        text = _sane(' '.join(segment['words']))
        if not text:
            continue
        if index:
...
def _is_stacked(block, frame):
    ...
    signer's designation is several short lines that merely sit close together. Nothing in a block
    of short lines ever reached the edge, so that is the test. Joining such a block into a
    paragraph runs its lines together, which is what turned a letterhead into one sentence.
    """
    if len(block['lines']) < 2:
        return False
    column = frame['right'] - frame['left']
    return column > 0 and block['x1'] < frame['left'] + FILLED_LINE * column
...
def _add_stacked_lines(paragraph, block, Pt):
    for index, line in enumerate(block['lines']):
        ...
        run.font.size = Pt(round(line['size'], 1)) if line['size'] else None
        if line['bold']:
            run.bold = True
        _apply_font(run, line['font'], line['text'])
